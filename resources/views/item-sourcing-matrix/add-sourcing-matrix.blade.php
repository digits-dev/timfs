...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form action="{{ CRUDBooster::mainpath('add-save') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
            ...
            <div class="row form-group">
                ...
                <div class="col-sm-5">
                    <select class="form-control" name="requestor_id" id="requestor_id">
                        <option value="" selected disabled>None selected...</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ in_array($user->id, $used_ids) ? 'disabled' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
<script>
    $('#requestor_id, #select_approvers').select2({
        with: '100%',
    });
