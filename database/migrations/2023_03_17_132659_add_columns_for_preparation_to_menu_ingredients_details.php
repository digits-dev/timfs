...
class AddColumnsForPreparationToMenuIngredientsDetails extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::table('menu_ingredients_details_temp', function (Blueprint $table) {
            DB::statement('alter table menu_ingredients_details_temp add column `prep_qty` float null after qty');
            DB::statement('alter table menu_ingredients_details_temp add column `menu_ingredients_preparations_id` int(10) null after uom_name');
            DB::statement('alter table menu_ingredients_details_temp add column `yield` float null after menu_ingredients_preparations_id');
            DB::statement('alter table menu_ingredients_details_temp add column `ttp` float null after yield');
        });
    }
